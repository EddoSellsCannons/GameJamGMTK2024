...
func update_chunks():
	...
	var player_position = $"../giantPlayer".global_position
	...
	for x in range(chunk_x - 4, chunk_x + 4):
		for y in range(chunk_y - 4, chunk_y + 4):
...
func generate_chunk(chunk_coords: Vector2):
	...
	for x in range(chunk_size):
		for y in range(chunk_size):
			...
			var xCoord = randi_range(0, 2)
			var yCoord = randi_range(0, 2)
